...
def log_token_usage(
    ...
    功能说明：
    - 记录每次 API 调用的 Token 消耗情况
    - 自动按项目和模型维度进行汇总统计
    - 生成 Markdown 格式的报告文件

    参数:
        project_name: 项目名称，用于分类统计
        model_name: 使用的模型名称
        prompt_tokens: 输入提示的 Token 数量
        completion_tokens: 模型输出的 Token 数量
...
def remove_surrogates(text: str) -> str:
    """
    【清理代理字符】
    移除字符串中的 UTF-16 代理字符，保证后续编码安全。

    背景说明：
    - 孤立的代理字符是非法 Unicode，会导致 JSON 序列化或文件编码错误
    - 常见于从 PDF 提取的文本或某些编码转换后的内容
